...
const AdminAccessDropdown = ({ id }) => {
  ...
  const handleDeleteAdminAccess = () => {
    if (confirm('Are you sure to delete this admin ?')) {
      const data = {
        id: id,
        isDeleted: true
      }
      axios.post(`${BASE_URL}/api/v1/admin/updateAdminbyId`, data, {
        headers: {
      ...
      .then((response) => {
        if (response.data.statusCode == 200) {
          toast.success('Admin Access Deleted');
          window.location = "/admin/admin-access"
        }
        else {
      ...
      <div className="relative" onClick={(event) => handleOpenDropdown(event)} onBlur={(event) => handleCloseDropdown(event)}>
        ...
        <div className={`absolute ${state} border right-0 z-50 mt-2 w-fit origin-top-right rounded bg-white border-slate-300 shadow-lg px-4 py-3`}>
          <ul className="flex flex-col space-y-3">
            <li><Link href={`/admin/admin-access/${id}`} className="text-xs font-medium text-slate-700 hover:text-ascent whitespace-nowrap flex items-center justify-start"> <TbEdit size={18} className="mr-1 mt-[-1px]" />Edit Access</Link></li>
            <li><a href="javascript:void(0);" onClick={() => handleDeleteAdminAccess()} className="text-xs font-medium text-red-500 hover:text-red-800 whitespace-nowrap flex items-center justify-start"> <TbTrash size={18} className="mr-1 mt-[-1px]" />Delete Access</a></li>
          </ul>
        </div>
      </div>
    </React.Fragment>
